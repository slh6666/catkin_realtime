...
namespace PathPlanningUtilities
{
	...
	inline std::istream& operator >> (std::istream& stream, Point2f& pt)
	{
		stream >> pt.x_;
	...
	inline std::istream& operator >> (std::istream& stream, Vector2f& vec)
	{
		stream >> vec.x_;
		if (stream.peek() == ',') stream.ignore();
		stream >> vec.y_;
	...
	inline std::istream& operator >> (std::istream& stream, CurvePoint& cpt)
	{
