...
class ThetaStarPlanner {
 public:
    // 构造函数
    ThetaStarPlanner(double robot_size){
        ...
        while (!open_set.empty()) {
            ...
            for (auto motion: this->motions_) {
                ...
                if (current_node.pre_index_ != -1) {
                    ...
                    // 得到父节点和邻居节点的连线
                    PathPlanningUtilities::LineSegment line_segment = PathPlanningUtilities::LineSegment(
                        PathPlanningUtilities::Point2f{static_cast<double>(parent_node.x_), static_cast<double>(parent_node.y_)}, 
                        PathPlanningUtilities::Point2f{static_cast<double>(neighbor_node.x_), static_cast<double>(neighbor_node.y_)});
                    // 判断连线是否发生碰撞
                    ...
                    if (!is_line_segment_collision) {
                        ...
                        // 计算损失
                        neighbor_node.cost_ = line_segment.length() + parent_node.cost_;
                        // 计算启发
