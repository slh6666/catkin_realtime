...
visualization_msgs::Marker VisualizationMethods::visualizeCurvesToMarker(const PathPlanningUtilities::Curve &curve, const std_msgs::ColorRGBA &color, int id, double fontsize) {
    ...
    curve_marker.header.frame_id = "odom";
...
// 将路径转换为marker array
visualization_msgs::MarkerArray VisualizationMethods::visualizeCurveToMarker(const nav_msgs::Path &path, double radius, const std_msgs::ColorRGBA &color) {
    visualization_msgs::MarkerArray path_marker_array;
    for (size_t i = 0; i < path.poses.size(); i++) {
        visualization_msgs::Marker pose_marker = visualizeSphere(path.poses[i].pose.position.x, path.poses[i].pose.position.y, radius, color, i);
        path_marker_array.markers.push_back(pose_marker);
    }
...
// 在特定位置生成圆球marker
visualization_msgs::Marker VisualizationMethods::visualizeSphere(double position_x, double position_y, double radius, const std_msgs::ColorRGBA &color, int id) {
