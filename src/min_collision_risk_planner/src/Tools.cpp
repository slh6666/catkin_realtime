...
// 计算曲线的长度
double Tools::calcCurveLength(const PathPlanningUtilities::Curve &curve) {
    double distance = 0;
    for (size_t i = 1; i < curve.size(); i++) {
        distance += PathPlanningUtilities::calcDistance(curve[i-1].position_, curve[i].position_);
    }
    return distance;
}

// 计算曲线每一个点的里程
std::vector<double> Tools::calcCurveDistances(const PathPlanningUtilities::Curve &curve) {
