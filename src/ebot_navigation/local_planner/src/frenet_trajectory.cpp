#include"../include/local_planner/frenet_trajectory.hpp"
...
void FrenetPathPlanner::Planning(){
    ...
    if(wx.size()>0 && reference_waypoints_x.size()>0 && reference_waypoints_y.size()>0){
        ...
        if(sqrt(pow(current_x - wx.back(), 2) + 
            ...
            //新增实时规划 根据当前定位确定求路径上的最近点更新当前状态
            double min_dist = std::numeric_limits<double>::max();
            int min_index = -1;
            for(int i = 0; i < final_path.x_.size(); i++){
                double dist = hypot(current_x - final_path.x_[i], current_y - final_path.y_[i]);  //current_x接收待测试
                if(dist < min_dist){
                    min_dist = dist;
                    min_index = i;
                }
            }
            std::cerr << "min dist to path point: " << min_dist << std::endl;
            if(min_dist < 0.1){ //0.1待测试
                current_s = final_path.s_[min_index];
                current_s_derivative = final_path.s_derivative_[min_index];
                current_s_2derivative = 0.0;    //为什么是0 ？？？
                current_d = final_path.d_[min_index];
                current_d_derivative= final_path.d_derivative_[min_index];
                current_d_2derivative = final_path.d_2derivative_[min_index];
            }
